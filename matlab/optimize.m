function opt_tf = optimize(icp)
    ...
    % (might need to double check matrix dims)    
    source_corr = icp.correspondences(:,1);
    target_corr = icp.correspondences(:,2);
    P = icp.source_pc(:,source_corr);
    X = icp.target_pc(:,target_corr);
    
    % From GetTransformation as part of Maani's SVD function (requires AA MATLAB package since it uses quaternion function)
